...
// Copy over the current wp themes
ncp('web/wp/wp-content/themes', 'web/wp-content/themes/', function(err) {
  if (err) {
    return console.error(err);
  }
  console.log('Copied wordpress themes into wp-content dir!');
});

// Create wp-config.php.
wp_cli( 'config create --dbname=wordpress_develop --dbuser=root --dbpass=password --dbhost=mysql --path=/var/www/web/wp --force' );
...
// Add custom folder paths
/**
 * Site url and home url. Remove these if you want to configure them in database.
 */
wp_cli( `config set WP_SITEURL "'http://localhost:${process.env.LOCAL_PORT}/wp'" --raw` );
wp_cli( `config set WP_HOME "'http://localhost:${process.env.LOCAL_PORT}'" --raw` );

/**
 * Custom content directory
 */
//wp_cli( `config set WP_CONTENT_DIR "__DIR__ . '/wp-content'" --raw` );
//wp_cli( `config set WP_CONTENT_URL "'http://localhost:${process.env.LOCAL_PORT}/wp-content'" --raw` );
